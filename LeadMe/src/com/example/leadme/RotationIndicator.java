...
/**
 * @author ccliu
 * UI组件类，展示感应器捕获到的手机姿态变化，姿态包括：偏角(yaw)，仰角(	pitch)，倾斜角(roll)
 */
public class RotationIndicator extends View{
	...
	private static final boolean LOG_FPS = false;
	  ...
	  public RotationIndicator(Context context, AttributeSet attrs) {
	    super(context, attrs);
	  ...
	  public void setAttitude(float yaw, float pitch, float roll) {
		mYaw = yaw;
	    mPitch = pitch;
	    mRoll = roll;
	    invalidate();
	  ...
	  private Bitmap getSrc() {
	    ...
	    canvas.rotate(mYaw, centerX, centerY);
	   ...
	   /* float bottomLadderStepX = mWidth / 12;
	    ...
	    float handRadius = mWidth / 3;
	    canvas.drawLine(centerX, centerY, centerX, centerY + handRadius, mHandPaint);
